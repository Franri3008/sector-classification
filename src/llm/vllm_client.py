...
class VLLMClient(LLMClient):
    def __init__(self, model: str | None = None, base_url: str | None = None) -> None:
        self.model_id = model or settings.llm.vllm_model
        self._base_url = (base_url or settings.llm.vllm_base_url).rstrip("/")
        self._client = None
    ...
    def _get_client(self):
        if self._client is None:
            ...
            self._client = OpenAI(api_key="not-needed", base_url=self._base_url)
        return self._client
    ...
    def _parse(self, system: str, user: str, schema: type[T]) -> T:
        ...
        if parsed is None:
            raise RuntimeError(
                f"vLLM returned null parsed response; refusal={resp.choices[0].message.refusal!r}"
            )
        return parsed
